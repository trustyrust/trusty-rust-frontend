    ...
    <div class="col">
      <section>
        ...
        <div class="text-body1">
          In this post, we will go through all the possible ways to solve the infamous FizzBuzz interview question in Rust. Rust is a
      ...
      <section>
        ...
        <div>Executing <text-code>cargo run</text-code> will predictably give you the following output:</div>
        <highlight-js
          language="shell"
          code="$ cargo run
...
fizzbuzz for number 20 is buzz"
      ...
      <section>
        ...
        <div class="text-body1">
          ...
          <text-code>mut</text-code> variables, which is best practice for reducing accidental bugs later on. It will look something like
          this:
        </div>
        ...
        <div class="text-body1">
          ...
          go down the match arms from top to bottom, and if it is both Fizz and Buzz, it will return <text-code>FizzBuzz</text-code>. If
          not, it will go to the second arm and check to match on <text-code>is_fizz</text-code>. If it is not
          <text-code>is_fizz</text-code>, it will go to the third arm and check if it matches <text-code>is_buzz</text-code>. If it is not
          <text-code>is_buzz</text-code>, it will go to the fourth arm, which will match on anything (similar to default in a case
          statement) and return the number as a <text-code>String</text-code>.
        </div>
        <div class="text-body1">
          This code is both more concise and more clear than <b>FizzBuzz using functions</b>. We also do not have any
          <text-code>mut</text-code> variables. Another benefit is that built into the Rust compiler is the requirement that all possible
          matches are handled. This means that if you were to forget to add a match arm, the compiler would reject your code with
          <text-code>non-exhaustive patterns</text-code> error.
        </div>
        <q-separator class="q-my-lg" />
      </section>
      <section></section>
      <section>
        ...
        <div class="text-body1">
          One of the key selling points of Rust is to have <b>Zero Cost Abstraction</b>, and using Generic Types is one of the ways to
          achieve this. You may have noticed that we defined our <text-code>FizzBuzz</text-code> function using a
          <text-code>u32</text-code> type. For all practical purposes, this would be sufficient, but for learning purposes, let's define the
          function using a generic type and call it with a <text-code>u128</text-code>
        </div>
...
fizzbuzz for number 12345678 is Fizz"
        />
        <div class="text-body1">
          You will notice a fair bit more complexity to this function, so let's break it down. <text-code>i</text-code> is defined as a
          generic type <text-code>T</text-code> in the function definition on line 3. Type <text-code>T</text-code> has two
          <text-code>constraints</text-code> that allow it to be treated like an integer. They are
        ...
        <div class="text-body1">
          ...
          <text-code>i128, u128, i64, u64, i32, u32, i16, u16, i8, u8, isize, usize</text-code>. To see more about these types, visit the
          <text-code>num</text-code> crate documentation
          <link-href link="https://docs.rs/num/0.4.0/num/trait.PrimInt.html" caption="PrimInt" />.
        </div>
        <div class="text-body1">
          As you can see, we also added the <text-code>Unsigned</text-code> constraint to restrict to only those types that are positive.
        </div>
        <div class="text-body1">
          On line 6, we added the <text-code>std::fmt::Display</text-code> constraint so that we can turn the integer back into a String
          type on line 19.
        </div>
        <div class="text-body1">
          Now when we run the code, we should be able to call this function with any unsigned integer type and Rust will be able to run
          without any additional overhead because Rust guarantees <b>Zero Cost Abstraction</b>.
        </div>
        <q-separator class="q-my-lg" />
      </section>
      <section>
        <div class="text-h4">FizzBuzz <text-code>impl</text-code> <text-code>From</text-code> trait</div>
        ...
        <div class="text-body2 q-pb-md">
          *Note: to reduce complexity you can see in line 12 that we implemented using type <text-code>u32</text-code> rather than defining
          a Generic Type but the same approach with generics could have been used.
        </div>
        <div class="text-body1">
          The first thing you will notice is that using <text-code>impl</text-code> feels much more like Object Oriented Programming. First
        ...
        <div class="text-body1 q-pt-md">
          ...
          <text-code>take_value</text-code>. These two functions act like <text-code>getters</text-code>. We have two here for demonstration
          purposes.
          <ul class="q-pt-sm">
            <li>
              <div class="text-body1">
                <text-code>borrow_value</text-code> will return a <text-code>&amp;str</text-code> or a borrowed reference to the
                <text-code>val</text-code> within the <text-code>struct</text-code>
              </div>
            </li>
            <li>
              <div class="text-body1">
            ...
            <li>
              <div class="text-body1">
                The <text-code>impl From&lt;u32&gt; for FizzBuzz</text-code> acts like our setter or constructor.
              </div>
              <div class="text-body1">
        ...
        <div class="text-body1">
          Now in our <text-code>main</text-code> function to turn our integer into our FizzBuzz struct, we simply have to call
        ...
        <div class="text-body1">A one-liner to get FizzBuzz from the integer 30 would look like:</div>
      ...
      <section>
        ...
        <div class="text-body1">
          As you see, Rust may be a compiled language, but it has a very flexible syntax which can allow you to write the same code in
          whatever style you are most comfortable with. In other posts, we will go into more of the details of
          <text-code>traits</text-code> and the power of implementing those traits to suit your needs. For now, enjoy knowing there are many
          ways to represent the FizzBuzz problem within the Rust language. The goal of this post is to demonstrate some of the nice features
          of Rust, such as an advanced pattern matching syntax, generic types, and the impl of a struct.
        </div>
